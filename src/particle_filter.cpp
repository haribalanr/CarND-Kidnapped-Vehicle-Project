 ...
 * Dynamic / ctrv model
 */
const vec_t ctrvModel(double delta_t, const vec_t& x, const vec_t& u, const double std_pos[])
{
  ...
  // Add noise
  px_p = px_p + dist_x(randgen);
  py_p = py_p + dist_y(randgen);
  pyaw_p = pyaw_p + dist_yaw(randgen);
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
  ...
  {
    auto p = particles[i];
    vec_t x_pred = ctrvModel(delta_t, {p.x, p.y, p.theta}, {velocity, yaw_rate}, std_pos);
...
const LandmarkObs toGlobal(const LandmarkObs& obs, const Particle& p)
{
  LandmarkObs out;
  // rotate local coordinates to the right orientation
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
  ...
  for(unsigned p_ctr=0; p_ctr < particles.size(); p_ctr++)
  {
    ...
    {
      auto obs_global = toGlobal(obs_lm, p);
